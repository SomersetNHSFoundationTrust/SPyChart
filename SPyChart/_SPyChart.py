...
class SPC:
    ...
    def __init__(self, df, target_col, chart_type='Individual-chart', change_dates=None, baseline_period=None):
        ...
        self.baseline_period = pd.to_datetime(self.baseline_period)
    ...
    def run_spc(self):
        ...
        self.setup()
        self.check_rules()
        start_date = pd.to_datetime(self.df.index.min()).strftime('%d/%m/%Y')
        end_date = pd.to_datetime(self.df.index.max()).strftime('%d/%m/%Y')
        title = f'<b>SPC Chart:</b> Monitoring the variable {self.target_col} during the period {start_date} to {end_date}'
